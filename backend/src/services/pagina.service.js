...
const generarPaginaSVG = (titulo, numeroCapitulo, orden, totalPaginas) => {
  ...
  const vinetas = maquetaDe(Number(orden) || 1)
    ...
    <rect x="${x}" y="${y}" width="${ancho}" height="${alto}" fill="#ffffff" stroke="#111111" stroke-width="4" rx="4"/>
    <rect x="${x}" y="${y}" width="${ancho}" height="${alto}" fill="${fondo}" fill-opacity="0.10" rx="4"/>
    <rect x="${x}" y="${y}" width="${ancho}" height="${alto}" fill="url(#vineta)" rx="4"/>
    <text x="${x + ancho / 2}" y="${y + alto / 2}" font-family="Arial Black" font-size="${Math.round(Math.min(ancho, alto) / 3)}" font-weight="900" fill="${fondo}" fill-opacity="0.18" text-anchor="middle" dominant-baseline="middle">${indice + 1}</text>`)
    .join('');
  ...
  return `<svg xmlns="http://www.w3.org/2000/svg" width="${ANCHO}" height="${ALTO}" viewBox="0 0 ${ANCHO} ${ALTO}">
    ...
    <text x="60" y="52" font-family="Arial" font-size="20" font-weight="bold" fill="#333333">${tituloSeguro}</text>
    <text x="${ANCHO - 60}" y="52" font-family="Arial" font-size="18" fill="#777777" text-anchor="end">Capitulo ${capituloSeguro}</text>
    <line x1="60" y1="64" x2="${ANCHO - 60}" y2="64" stroke="${acento}" stroke-width="2"/>${vinetas}
    <text x="${ANCHO / 2}" y="${ALTO - 24}" font-family="Arial" font-size="18" fill="#999999" text-anchor="middle">${pieDePagina}</text>
  </svg>`;
};
